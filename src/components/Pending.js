...
export default function Pending() {
  ...
  const deletePending = (deleteId) => {
    axios.delete('http://localhost:8080/pending/' + deleteId, {headers : {"Authorization" : localStorage.getItem('tokenType') + " " + localStorage.getItem('token')}})
    .then((Response)=>{
  ...
  const serachData = () => {
    setPage (0)
    if(searchString == ''){
      axios.get("http://localhost:8080/pending/page", {headers : {"Authorization" : localStorage.getItem('tokenType') + " " + localStorage.getItem('token')}})
      .then((Response) => {
    ...
    useEffect(() => {
      // setBooks(["Hello"])
      // console.log("books");
      axios.get("http://localhost:8080/pending/page?pageNo="+page, {headers : {"Authorization" : localStorage.getItem('tokenType') + " " + localStorage.getItem('token')}})
        .then((Response) => {
    ...
    const handleChangePage = (event, newPage) => {
      setPage(newPage);
      if(searchString == ''){
      axios.get("http://localhost:8080/pending/page?pageNo="+newPage, {headers : {"Authorization" : localStorage.getItem('tokenType') + " " + localStorage.getItem('token')}})
      .then((Response) => {
        //console.log(Response.data.Total_No_Of_Elements)
        // setCount (Response.data.Total_No_Of_Elements)
        // console.log(count)
      ...
      <Table className={classes.table} size="small">
        <TableHead>
          <TableRow>
            ...
            <TableCell align="left"><b>Artist Name</b></TableCell>
        ...
        <TableBody>
          {pendings.map((row) => (
            ...
            <TableRow>
              ...
              <TableCell align="left">{row.upload.title}</TableCell>
              <TableCell align="left">{row.upload.artistName}</TableCell>
              <TableCell align="left">{row.upload.description}</TableCell>
              <TableCell align="left">{row.upload.price}</TableCell>
              ...
              <TableCell align="left">{row.upload.sellerId}</TableCell>
            </TableRow>
          ))}
        </TableBody>
        <TableFooter>
