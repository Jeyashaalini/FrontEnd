...
export default function UploadDetails(props) {
  ...
  const deleteUpload = (deleteId) => {
    axios.delete('http://localhost:8080/upload/' + deleteId,)
    .then((deleteId) => {
  ...
  const serachData = () => {
    setPage(0)
    if(searchString === ''){
      axios.get("http://localhost:8080/upload/page")
      .then((Response) => {
      ...
      axios.get("http://localhost:8080/upload/page/serachedPages?serched="+searchString)
      .then((Response) => {
        setCount (Response.data.Total_No_Of_Elements)
        console.log(count)
      setUpload( Response.data.data)
    ...
    useEffect(() => {
      // setBooks(["Hello"])
      // console.log("books");
      axios.get("http://localhost:8080/upload/page?pageNo="+page)
        .then((Response) => {
    ...
    const handleChangePage = (event, newPage) => {
      setPage(newPage);
      if(searchString === ''){
      axios.get("http://localhost:8080/upload/page?pageNo="+newPage)
      .then((Response) => {
        console.log(Response.data.Total_No_Of_Elements)
        setCount (Response.data.Total_No_Of_Elements)
        console.log(count)
      ...
      <Table className={classes.table} size="large">
        <TableHead>
          <TableRow>
            ...
            <TableCell align="left"><b>Medium</b></TableCell>
